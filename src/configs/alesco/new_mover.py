def config(env: str, partition_value: str) -> dict:
    if env in ['dev', 'qa']:
        sftp_source = '/Inbox/Sample/alesco/new_mover/'
        ncoa_request_dir = '/Inbox/Sample/alesco/new_mover/request/'
        ncoa_response_dir = '/Inbox/Sample/alesco/new_mover/request/'
    elif env == 'preprod':
        sftp_source = '/Inbox/alesco/new_mover/'
        ncoa_request_dir = '/Inbox/alesco/new_mover/request/'
        ncoa_response_dir = '/Inbox/alesco/new_mover/request/'
    elif env == 'prod':
    ...
    'life_events_calculated_fields': [
        ('fmcgid', f'F.concat(F.lit("ANM"), F.lit(datetime.strptime("{partition_value}","%Y%m%d").strftime("%m%d%y")), F.lpad(F.row_number().over(Window.orderBy(F.monotonically_increasing_id())), 7, "0"))'),
        ('dlx_recordid', f'F.concat(F.lit("ANM"), F.lit(datetime.strptime("{partition_value}","%Y%m%d").strftime("%m%d%y")), F.lpad(F.row_number().over(Window.orderBy(F.monotonically_increasing_id())), 7, "0"))'),
        ('prev_address', 'F.concat_ws(" ", F.col("oldaddress1"), F.col("oldaddress2"))'),
        ('prev_city', 'F.col("oldcity")'),
        ('prev_state', 'F.col("oldstate")'),
        ('prev_zip', 'F.when((F.col("oldzip")).isNotNull() & (F.trim(F.col("oldzip")) != ""), F.lpad(F.trim(F.col("oldzip")), 5, "0")).otherwise(F.lit(None).cast(StringType()))'),
        ...
        ('dwelling_type', 'F.when((F.col("dwellingtype") == "A"), "MFDU").when((F.col("dwellingtype") == "C"), "MFDU").when((F.col("dwellingtype") == "H"), "SFDU").when((F.col("dwellingtype") == "T"), "SFDU").otherwise("")'),
    ...
    'core_table_name': 'consumer_alesco_new_mover',
    'check_names':True,
    'bypass_file': False,
    'parameter_store':'dlx-ddm-sterling-sftp-creds',
    'melissa_parameter_store':'dlx-ddm-melissa-sftp-creds',
    'source_filename':'AlescoNewMov_YYYYMMDD.csv',
    'source_extract_filename':'AlescoNewMov_YYYYMMDD.csv',
    'source_count_filename':'AlescoNewMov_YYYYMMDD_count.txt',
