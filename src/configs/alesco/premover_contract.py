def config(env: str, partition_value: str) -> dict:
    if env in ['dev', 'qa']:
        sftp_source = '/Inbox/Sample/alesco/premover_contract/'
        ncoa_request_dir = '/Inbox/Sample/alesco/premover_contract/request/'
        ncoa_response_dir = '/Inbox/Sample/alesco/premover_contract/request/'
    elif env == 'preprod':
        sftp_source = '/Inbox/alesco/premover_contract/'
        ncoa_request_dir = '/Inbox/alesco/premover_contract/request/'
        ncoa_response_dir = '/Inbox/alesco/premover_contract/request/'
    elif env == 'prod':
    ...
    'life_events_calculated_fields': [
        ('fmcgid', f'F.concat(F.lit("ALC"), F.lit(datetime.strptime("{partition_value}","%Y%m%d").strftime("%m%d%y")), F.lpad(F.row_number().over(Window.orderBy(F.monotonically_increasing_id())), 7, "0"))'),
        ('dlx_recordid', f'F.concat(F.lit("ALC"), F.lit(datetime.strptime("{partition_value}","%Y%m%d").strftime("%m%d%y")), F.lpad(F.row_number().over(Window.orderBy(F.monotonically_increasing_id())), 7, "0"))'),
        ...
        ('source', 'F.lit("ALESCO_PREMOVERCONTRACT")'),
        ('vendor', 'F.lit("Alesco")'),
        ('source_type', 'F.lit("Premover Contract")'),
        ('listing_price', 'F.when((F.col("listingprice").isNotNull()) & (F.trim(F.col("listingprice")) != ""),  F.col("listingprice")).otherwise(F.lit(None).cast(StringType()))'),
    ...
    'core_table_name': 'consumer_alesco_premover_contract',
    'check_names':True,
    'bypass_file': False,
    'parameter_store':'dlx-ddm-sterling-sftp-creds',
    'melissa_parameter_store':'dlx-ddm-melissa-sftp-creds',
    'source_filename':'AlescoPendMov_YYYYMMDD.csv',
    'source_extract_filename':'AlescoPendMov_YYYYMMDD.csv',
    'source_count_filename':'AlescoPendMov_YYYYMMDD_count.txt',
