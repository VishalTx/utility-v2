def config(env: str, partition_value: str) -> dict:
    if env in ['dev', 'qa']:
        sftp_source = '/Inbox/Sample/alesco/newly_engaged/'
        ncoa_request_dir = '/Inbox/Sample/alesco/newly_engaged/request/'
        ncoa_response_dir = '/Inbox/Sample/alesco/newly_engaged/request/'
    elif env == 'preprod':
        sftp_source = '/Inbox/alesco/newly_engaged/'
        ncoa_request_dir = '/Inbox/alesco/newly_engaged/request/'
        ncoa_response_dir = '/Inbox/alesco/newly_engaged/request/'
    elif env == 'prod':
    ...
    'life_events_calculated_fields': [
        ('fmcgid', f'F.concat(F.lit("ANE"), F.lit(datetime.strptime("{partition_value}","%Y%m%d").strftime("%m%d%y")), F.lpad(F.row_number().over(Window.orderBy(F.monotonically_increasing_id())), 7, "0"))'),
        ('dlx_recordid', f'F.concat(F.lit("ANE"), F.lit(datetime.strptime("{partition_value}","%Y%m%d").strftime("%m%d%y")), F.lpad(F.row_number().over(Window.orderBy(F.monotonically_increasing_id())), 7, "0"))'),
        ...
        ('phone', 'F.when((F.col("telephonenumber") == "0000000000"), "").otherwise(F.col("telephonenumber"))'),
        ...
        ('source', 'F.lit("ALESCO_NEWLYENGAGED")'),
        ('vendor', 'F.lit("Alesco")'),
        ...
        ('weddingdate', 'F.col("weddingdate")'),
    ],

    'data_category': 'consumer',
    'core_table_name': 'consumer_alesco_newly_engaged',
    'check_names':True,
    'bypass_file': False,
    'parameter_store':'dlx-ddm-sterling-sftp-creds',
    'melissa_parameter_store':'dlx-ddm-melissa-sftp-creds',
    'source_filename':'AlescoNewEngaged_YYYYMMDD.csv',
    'source_extract_filename':'AlescoNewEngaged_YYYYMMDD.csv',
    'source_count_filename':'AlescoNewEngaged_YYYYMMDD_count.txt',
