def config(env: str, partition_value: str) -> dict:
    if env in ['dev', 'qa']:
        sftp_source = '/Inbox/Sample/alesco/new_homeowner/'
        ncoa_request_dir = '/Inbox/Sample/alesco/new_homeowner/request/'
        ncoa_response_dir = '/Inbox/Sample/alesco/new_homeowner/request/'
    elif env == 'preprod':
        sftp_source = '/Inbox/alesco/new_homeowner/'
        ncoa_request_dir = '/Inbox/alesco/new_homeowner/request/'
        ncoa_response_dir = '/Inbox/alesco/new_homeowner/request/'
    elif env == 'prod':
    ...
    'life_events_calculated_fields': [
        ('fmcgid', f'F.concat(F.lit("ANH"), F.lit(datetime.strptime("{partition_value}","%Y%m%d").strftime("%m%d%y")), F.lpad(F.row_number().over(Window.orderBy(F.monotonically_increasing_id())), 7, "0"))'),
        ('dlx_recordid', f'F.concat(F.lit("ANH"), F.lit(datetime.strptime("{partition_value}","%Y%m%d").strftime("%m%d%y")), F.lpad(F.row_number().over(Window.orderBy(F.monotonically_increasing_id())), 7, "0"))'),
        ...
        ('source', 'F.lit("ALESCO_NEWHOMEOWNER")'),
        ('vendor', 'F.lit("Alesco")'),
        ('source_type', 'F.lit("New Homeowner")'),
    ...
    'core_table_name': 'consumer_alesco_new_homeowner',
    'check_names':True,
    'bypass_file': False,
    'parameter_store':'dlx-ddm-sterling-sftp-creds',
    'melissa_parameter_store':'dlx-ddm-melissa-sftp-creds',
    'source_filename':'AlescoNewHO_YYYYMMDD.csv',
    'source_extract_filename':'AlescoNewHO_YYYYMMDD.csv',
    'source_count_filename':'AlescoNewHO_YYYYMMDD_count.txt',
