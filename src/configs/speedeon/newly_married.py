def config(env: str, partition_value: str) -> dict:
    """
    Description : Configuration file for Speedon newly married
    :param env: environment
    :param partition_value: data processing date
    :return:
    """
    if env in ['dev', 'qa']:
        sftp_source = '/Inbox/Sample/speedeon/newly_married/'
        ncoa_request_dir = '/Inbox/Sample/speedeon/newly_married/request/'
        ncoa_response_dir = '/Inbox/Sample/speedeon/newly_married/request/'
    elif env == 'preprod':
        sftp_source = '/Inbox/speedeon/newly_married/'
        ncoa_request_dir = '/Inbox/speedeon/newly_married/request/'
        ncoa_response_dir = '/Inbox/speedeon/newly_married/request/'
    elif env == 'prod':
    ...
    'life_events_calculated_fields': [
        ('fmcgid', f'F.concat(F.lit("SPM"), F.lit(datetime.strptime("{partition_value}","%Y%m%d").strftime("%m%d%y")), F.lpad(F.row_number().over(Window.orderBy(F.monotonically_increasing_id())), 7, "0"))'),
        ('dlx_recordid', f'F.concat(F.lit("SPM"), F.lit(datetime.strptime("{partition_value}","%Y%m%d").strftime("%m%d%y")), F.lpad(F.row_number().over(Window.orderBy(F.monotonically_increasing_id())), 7, "0"))'),
        ...
        ('source', 'F.lit("SPEEDEON_NEWMARRIED")'),
        ('vendor', 'F.lit("SPEEDEON")'),
        ('source_type', 'F.lit("Newly Married")'),
    ...
    'core_table_name': 'consumer_speedeon_newly_married',
    'check_names':True,
    'parameter_store':'dlx-ddm-sterling-sftp-creds',
    'melissa_parameter_store':'dlx-ddm-melissa-sftp-creds',
    'source_filename':'FMCG_NewlyMarried_YYYYMMDD.txt',
    'source_extract_filename':'FMCG_NewlyMarried_YYYYMMDD.txt',
    'source_count_filename':'FMCG_NewlyMarried_YYYYMMDD_count.txt',
    'bypass_file':'N',
