def config(env: str, partition_value: str) -> dict:
    """
    Description : Configuration file for Speedon Newly engaged
    :param env: environment
    :param partition_value: data processing date
    :return:
    """
    if env in ['dev', 'qa']:
        sftp_source = '/Inbox/Sample/speedeon/newly_engaged/'
        ncoa_request_dir = '/Inbox/Sample/speedeon/newly_engaged/request/'
        ncoa_response_dir = '/Inbox/Sample/speedeon/newly_engaged/request/'
    elif env == 'preprod':
        sftp_source = '/Inbox/speedeon/newly_engaged/'
        ncoa_request_dir = '/Inbox/speedeon/newly_engaged/request/'
        ncoa_response_dir = '/Inbox/speedeon/newly_engaged/request/'
    elif env == 'prod':
    ...
    'life_events_calculated_fields': [
        ('fmcgid', f'F.concat(F.lit("SPE"), F.lit(datetime.strptime("{partition_value}","%Y%m%d").strftime("%m%d%y")), F.lpad(F.row_number().over(Window.orderBy(F.monotonically_increasing_id())), 7, "0"))'),
        ('dlx_recordid', f'F.concat(F.lit("SPE"), F.lit(datetime.strptime("{partition_value}","%Y%m%d").strftime("%m%d%y")), F.lpad(F.row_number().over(Window.orderBy(F.monotonically_increasing_id())), 7, "0"))'),
        ...
        ('source', 'F.lit("SPEEDEON_NEWENGAGED")'),
        ('vendor', 'F.lit("SPEEDEON")'),
        ('source_type', 'F.lit("Newly Engaged")'),
    ...
    'core_table_name': 'consumer_speedeon_newly_engaged',
    'check_names':True,
    'parameter_store':'dlx-ddm-sterling-sftp-creds',
    'melissa_parameter_store':'dlx-ddm-melissa-sftp-creds',
    'source_filename':'FMCG_NewlyEngaged_YYYYMMDD.txt',
    'source_extract_filename':'FMCG_NewlyEngaged_YYYYMMDD.txt',
    'source_count_filename':'FMCG_NewlyEngaged_YYYYMMDD_count.txt',
    'bypass_file':'N',
