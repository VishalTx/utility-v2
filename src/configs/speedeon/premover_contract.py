def config(env: str, partition_value: str) -> dict:
    """
    Description : Configuration file for Speedon Premover Contract
    :param env: environment
    :param partition_value: data processing date
    :return:
    """
    if env in ['dev', 'qa']:
        sftp_source = '/Inbox/Sample/speedeon/premover_contract/'
        ncoa_request_dir = '/Inbox/Sample/speedeon/premover_contract/request/'
        ncoa_response_dir = '/Inbox/Sample/speedeon/premover_contract/request/'
    elif env == 'preprod':
        sftp_source = '/Inbox/speedeon/premover_contract/'
        ncoa_request_dir = '/Inbox/speedeon/premover_contract/request/'
        ncoa_response_dir = '/Inbox/speedeon/premover_contract/request/'
    elif env == 'prod':
    ...
    'life_events_calculated_fields': [
        ('fmcgid', f'F.concat(F.lit("SPC"), F.lit(datetime.strptime("{partition_value}","%Y%m%d").strftime("%m%d%y")), F.lpad(F.row_number().over(Window.orderBy(F.monotonically_increasing_id())), 7, "0"))'),
        ('dlx_recordid', f'F.concat(F.lit("SPC"), F.lit(datetime.strptime("{partition_value}","%Y%m%d").strftime("%m%d%y")), F.lpad(F.row_number().over(Window.orderBy(F.monotonically_increasing_id())), 7, "0"))'),
        ...
        ('source', 'F.lit("SPEEDEON_PREMOVERCONTRACT")'),
        ('vendor', 'F.lit("SPEEDEON")'),
        ('source_type', 'F.lit("Premover Contract")'),
    ...
    'core_table_name': 'consumer_speedeon_premover_contract',
    'check_names':True,
    'parameter_store':'dlx-ddm-sterling-sftp-creds',
    'melissa_parameter_store':'dlx-ddm-melissa-sftp-creds',
    'source_filename':'FMCG_PremoveratContract_YYYYMMDD.txt',
    'source_extract_filename':'FMCG_PremoveratContract_YYYYMMDD.txt',
    'source_count_filename':'FMCG_PremoveratContract_YYYYMMDD_count.txt',
    'bypass_file':'N',
    ...
    'quality_checks':{'fname':'Completeness','lname':'Completeness','address':'Completeness','city':'Completeness'},
